...
def _prepare_frame(path: Path, request: StackRequest, reference: np.ndarray, calibration, size: tuple[int, int], foreground: np.ndarray | None) -> np.ndarray:
    frame = read_linear_rgb(path, request.options.half_size)
...
def stack_images(request: StackRequest) -> StackResult:
    lights = sorted((Path(path).resolve() for path in request.lights), key=acquisition_sort_key)
    darks = sorted((Path(path).resolve() for path in request.darks), key=acquisition_sort_key)
    if not lights:
        raise ValueError("select at least one light frame")
    missing = [path for path in (*lights, *darks) if not path.is_file()]
    if missing:
    ...
    reference = read_linear_rgb(reference_path, request.options.half_size)
    size = reference.shape[1], reference.shape[0]
    calibration = build_master_dark(darks, request.options.half_size, size, request.progress)
    ...
    auto_foreground = request.options.protect_foreground and request.options.foreground_mask is None
    if request.options.protect_foreground:
        if request.options.foreground_mask:
            foreground = read_mask(request.options.foreground_mask, size)
